---
name: supervisor
description: 複数のワーカープロジェクトを束ねて俯瞰計画し、各ワーカーの interrupt 受信箱へ指示を配信し、ワーカーからのエスカレーションを取り込む。supervisor として動くのは `.workers` 宣言があるプロジェクトのみ（無ければ何もしない＝暴発しない）。優先度は要求側の要望を踏まえつつ supervisor が確定する。ユーザーが「supervisor」「workers をまとめる」「配下に指示」「俯瞰計画」「エスカレーションを捌く」等を求めたときに使う。プロジェクト非依存。
---
...
> ロール（supervisor/worker）はアイデンティティではなく**構造と権限**で決まる。`.workers` を宣言した
> プロジェクトだけが supervisor として振る舞える（＝capability の昇格）。ワーカーは普通のプロジェクトで、
> 自分が誰の配下かを基本は知らない。

## 0. 起動時チェック（supervisor か判定）

プロジェクト直下の **`.workers`** を読む。

- **無い／空** → **supervisor 文脈ではない。このスキルは何もしない**（ワーカーとして [[memory-bank]] を使う）。
- **ある** → 列挙された各ワーカーを配下とする。各ワーカーの `<worker>/.work/skills/memory-bank/` の
  `activeContext.md` / `progress.md` を読んで状態を把握してから計画する。

### `.workers` 形式

プロジェクト直下のファイル。1 行 1 ワーカー（プロジェクトのパス。相対 or 絶対）。`#` でコメント。

```
# このプロジェクトを supervisor にする宣言。配下ワーカーのパスを列挙。
workers/manystore
workers/dotfiles
```

> 物理配置は `workers/` 配下に置く運用が分かりやすいが必須ではない（`.workers` のパスが正本）。
> **宣言しないと動かない**＝明示的にワーカーを与えたプロジェクトだけが supervisor になる。

## コアワークフロー

```
Start → .workers 判定 → 自分の interrupt を取り込み（ワーカーからのエスカレ）
...
4. **配信（下り）** — 指示は対象ワーカーの **`<worker>/.work/skills/memory-bank/interrupt/`** にメッセージを
   ...
   サブエージェントを起動する（同期）。
5. **境界** — 触れてよいのは **`.workers` に挙げた配下のみ**。それ以外の外部プロジェクトには手を出さない。
...
- `.workers` が無ければ**何もしない**（ワーカー単体で誤って supervisor 化しない）。
