...
| **R10 検証規約の取り込み** | [[flow]] を使う repo は、その `techContext.md`「検証コマンド」が **本スキル（quality）を参照し materialize**（`make check` 等の Makefile 呼び出し）していること。生 `uvx ruff …` のベタ書きや、R5/R8 と drift した記述が無い | `techContext.md`「検証コマンド」節を確認（[[unit-quality]] への言及・`make` 経由・ベタ書き不在）。**supervisor が配下 worker（と自身）に対し定期実行**（[[role]]）。判断（あるべき姿）は本スキルが正本 |
...
- **R10（検証規約の取り込み）の判断は本スキルが正本だが、定期実行は [[role]] が担う**（配下 worker の
