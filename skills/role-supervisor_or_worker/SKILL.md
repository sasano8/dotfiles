...
ロール間の権限は**非対称**。「互いの repo に触れない」完全疎結合ではなく、**向きで許可が変わる**:

- **下り（supervisor → worker）は許可**。supervisor は全体俯瞰の知識を持つので配下へ干渉してよい——
  指示の dispatch、worker の `interrupt/` への投函、worker の `outbox/` を回収して受領印を付ける編集、即時に
  動かしたいときの worker dir 指定サブエージェント起動など。完全疎結合は非現実的で、box の受領編集のような
  下りの接触は前提として起きる。
- **上り（worker → supervisor）は禁止＝越権**。worker は親（supervisor）の正本・受信箱に**直接書き込まない**。
  用があるときは自分の `outbox/` に積むだけ（[[flow]] の pull 型エスカレ）で、supervisor が回収する。この上りの
  直接書き込みは **supervisor repo 側の編集ガード（個別層）** が決定論的に deny する（具体の実装・配線はその repo の
  関心事＝下記「境界の強制」。スキルは原則だけ持つ）。

> 「worker 不可侵（supervisor も worker に触れない）」ではない——触れてよいのは**下りだけ**。即時フィードバック
> （worker を前に進める干渉）も下りなので許可してよい（疎結合より開発速度を優先する場面がある）。
...
上の**上り禁止**（worker は親の正本を直接編集しない）は口約束では守られない（記憶依存ですり抜ける。実例:
worker セッションが共有スキルの symlink 越しに親のスキル正本を書き換え、親に WIP が残った）。これは
**supervisor repo 側の編集ガード**（起動セッション単位で効く PreToolUse 等のフック）で毎回・決定論的に止めるのが
正しい（**止めるのは上りだけ**。下り＝supervisor 自身の自己編集・worker 内の自分のファイル編集は素通し）。満たすべき要件:
...
```
Start → workers_dir 宣言の判定 → 各ワーカーの outbox を回収（pull 型・上りエスカレ）
     → 各ワーカーの状態を把握 → 俯瞰計画 → 優先度を確定 → 指示を配信 → 記録
```

1. **エスカレ回収（pull）** — 各ワーカーの `<worker>/.work/skills/flow-memory-bank/outbox/` を [[flow]] の規約で
   回収する（worker は親を知らず自分の outbox に積むだけ＝pull 型）。回収したら受領印を付ける（下り権限）。
   ※ 自分の `interrupt/` は**自分宛て**の入力（ユーザー／非対話の放り込み）を取り込む別経路。
2. **状態把握** — 配下各ワーカーの memory-bank を読む（何が進行中か／詰まっているか）。あわせて
   **quality 取り込みの drift の兆候を軽く拾う**（下記「quality は下り dispatch で」）。
...
4. **配信（下り）** — 指示は対象ワーカーの **`<worker>/.work/skills/flow-memory-bank/interrupt/`** にメッセージを
   ...
   サブエージェントを起動する（同期）。この dispatch は worker の [[flow]] 開発内ループの**起点**になり得るが、
   supervisor は起点を投げるだけで各反復は driver しない（疎結合）。
5. **境界（下り許可・上り禁止）** — 下りで触れてよいのは **`workers_dir` 配下のワーカーのみ**。それ以外の外部
   プロジェクトには手を出さない（上記「役割の権限」）。
...
## quality は下り dispatch で（横断監査しない）

[[unit-quality]] は判断（あるべき姿）の正本を持つが**オンデマンドで自走しない**。supervisor は配下の品質を
**自分で横断監査しない**——**組織の品質方針**（supervisor memory-bank `reference/quality-policy.md`）の取り込み
drift（各 worker の `techContext.md` が `make check` 等を materialize しているか＝[[unit-quality]] R10）も含め、
気になれば対象 worker の `interrupt/` へ「自己点検せよ（[[flow]] の開発内ループで [[unit-quality]] を回せ）」と
**下り dispatch するだけ**。実点検は worker 側の flow→unit で走る。

- これで **role→flow→unit が一貫**する（role→unit の直参照・横断定期スイープは持たない＝両建てにしない）。
- supervisor は worker の roll-up（outbox／状態把握）で drift の兆候を拾い、必要なら上記の dispatch をするだけ。
  「ズレているか」の判断基準（R10/R5/R8）は [[unit-quality]] が正本。
- メタ作業に偏らない原則（上記「優先度の判断」）に従い、drift の兆候が無ければ何もしない。
